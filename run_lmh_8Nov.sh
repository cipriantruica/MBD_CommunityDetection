...
# this code is used for computing louvain modularity for the 8th of November
...
#alpha=0.001
spark-submit --master $MASTER --deploy-mode $MODE --num-executors $NUM_EXECS --executor-cores $NUM_CORES --executor-memory $MEM_EXECS --class Driver $JAR_FILE $DATE $ALPHA3 $ECF $NO_TBL 1 >> "results/output_LMH_"$NO_TBL"TBL_"$DATE"_alpha_"$ALPHA3"_ECF_"$ECF
sleep 10

#alpha=0.01
spark-submit --master $MASTER --deploy-mode $MODE --num-executors $NUM_EXECS --executor-cores $NUM_CORES --executor-memory $MEM_EXECS --class Driver $JAR_FILE $DATE $ALPHA2 $ECF $NO_TBL 1 >> "results/output_LMH_"$NO_TBL"TBL_"$DATE"_alpha_"$ALPHA2"_ECF_"$ECF
sleep 10
